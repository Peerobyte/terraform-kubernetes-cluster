...
# Set hostname
hostnamectl set-hostname k8s-master-${count}
echo "127.0.0.1 k8s-master-${count}" >> /etc/hosts
...
# Join the cluster
# Wait for the master to be ready (join-master.sh available)
echo "[INFO] Waiting for join-master.sh to become available on the master node..."
until curl -sf http://${master_ip}/join-master.sh > /dev/null; do
  echo "[INFO] Master not ready, retrying in 10 seconds..."
  sleep 10
done

echo "[INFO] Master is ready. Proceeding to join the cluster..."
curl -s http://${master_ip}/join-master.sh | bash
